...
contract VotingSystem is Ownable, ReentrancyGuard {
    ...
    address public fundingPool;           // Address of FundingPool
    address public grantManager;         // Address of GrantManager
    uint256 private _roundIdCtr;        // Auto-incrementing round ID

    IIdeaRegistry public ideaI;
    ...
    /* ========== CONSTRUCTOR ========== */
    constructor(address _governanceToken, address _ideaRegistry) Ownable(msg.sender) {
        ...
        ideaI = IIdeaRegistry(_ideaRegistry);

        _roundIdCtr = 1; // Start round IDs from 1
    }

    /* ========== MODIFIERS ========== */
    modifier roundExists(uint256 roundId) {
        require(roundId > 0 && roundId < _roundIdCtr, "round not exist");
        _;
    }
    ...
    function startVotingRound(uint256[] calldata ideaIds) external onlyOwner returns (uint256) {
        ...
        _roundIdCtr++; // Prepare for next round
        return newId;
    }
    ...
    function setFundingPool(address _pool) external onlyOwner {
        require(_pool != address(0), "pool 0");
        fundingPool = _pool;
    }

    function setGrantManager(address _mgr) external onlyOwner {
        require(_mgr != address(0), "mgr 0");
        grantManager = _mgr;
    }
    ...
    /** @notice Should update idea status in IdeaRegistry.sol */
    function updateIdeaStatus(uint256 id, uint8 status) external {
        ideaI.updateStatus(id, status);
    }
